...
parser.add_argument("-t", "--timeout", help="Set the timeout of per IP scan (Recommend 0)")
...
if direct == 0:
	...
	for i in range(minRange, maxRange+1):
		sql = 'fping -a -t ' + timeout + ' ' + str(i) + '.' + str(ipArray[1]) + '.' + str(ipArray[2]) + '.' + str(ipArray[3])
	...
	for i in range(minRange, maxRange+1):
		sql = 'fping -a -t ' + timeout + ' ' + str(ipArray[0]) + '.' + str(i) + '.' + str(ipArray[2]) + '.' + str(ipArray[3])
		(status, output) = commands.getstatusoutput(sql)
		if output.find('ICMP') == -1 and output != '':
			list.append(str(output))
			if args.detial:
	...
	for i in range(minRange, maxRange+1):
		sql = 'fping -a -t ' + timeout + ' ' + str(ipArray[0]) + '.' + str(ipArray[1]) + '.' + str(i) + '.' + str(ipArray[3])
		(status, output) = commands.getstatusoutput(sql)
		if output.find('ICMP') == -1 and output != '':
			list.append(str(output))
			if args.detial:
	...
	for i in range(minRange, maxRange+1):
		sql = 'fping -a -t ' + timeout + ' ' + str(ipArray[0]) + '.' + str(ipArray[1]) + '.' + str(ipArray[2]) + '.' + str(i)
		(status, output) = commands.getstatusoutput(sql)
		if output.find('ICMP') == -1 and output != '':
			list.append(str(output))
			if args.detial:
